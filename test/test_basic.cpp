...
int main(int argc, char **argv) {
	...
	try {
		...
		std::cout << "===== CppDB Basic Test Report =====\n";
		std::cout << "Version: " << cppdb::version_string() << " (" << cppdb::version_number() << ")\n";
		...
		if (sql.engine() == "sqlite3") {
			sql << "create table test ( id integer primary key autoincrement not null, "
				   "n integer, f real , t timestamp ,name text )"
				<< cppdb::exec;
			...
			sql << "create table test ( id  serial  primary key not null "
				   ",n integer, f double precision , t timestamp ,name text )"
				<< cppdb::exec;
		} else if (sql.engine() == "mssql") {
			sql << "create table test ( id integer identity(1,1) primary key not null "
				   ",n integer, f double precision , t datetime ,name text )"
				<< cppdb::exec;
		} else {
		...
		long long rowid;
		{
			cppdb::statement stat = sql << "insert into test(n,f,t,name) values(?,?,?,?)" << 10 << 3.1415926565 << t
										<< "Hello \'World\'";
			stat.exec();
		...
		std::cout << "[DELETE] affected=" << stat.affected() << '\n';
		...
		std::cout << "[ONCE] call_counter=" << call_counter << '\n';
